...
import { updateExpenseCategory, deleteExpenseCategory } from "@/lib/modules/company-settings/service";
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const result = await updateExpenseCategory(session, params.id, parsed.data.name);
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const result = await deleteExpenseCategory(session, params.id);
  return serviceResultToResponse(result);
}
